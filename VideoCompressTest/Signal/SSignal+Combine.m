...
+ (SSignal *)combineSignals:(NSArray *)signals withInitialStates:(NSArray *)initialStates
{
    return [[SSignal alloc] initWithGenerator:^(SSubscriber *subscriber)
    {
        ...
        SAtomic *combineStateHolder = [[SAtomic alloc] initWithValue:[[SSignalCombineState alloc] initWithLatestValues:initialLatestValues completedStatuses:completedStatuses error:false]];
        ...
        for (SSignal *signal in signals)
        {
            ...
            {
                SSignalCombineState *currentState = [combineStateHolder modify:^id(SSignalCombineState *state)
                {
            ...
            error:^(id error)
            {
                ...
                [combineStateHolder modify:^id(SSignalCombineState *state)
                {
